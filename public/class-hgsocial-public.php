...
class hgSocial {
	 ...
	 * @var		string
	 */
	protected $plugin_slug = 'hg-social-share';
	...
	*/
	public function register_scripts() {
		...
		$success = ( self::opt('us_mail_success', 'us_mail') ) ? self::opt('us_mail_sucess', 'us_mail'): __('Great work! Your message was sent.', 'hg-social-share' );
		...
		$total_shares_text = ( self::opt('us_total_shares_text', 'us_basic') ) ? self::opt('us_total_shares_text', 'us_basic'): __( 'Total Shares', 'hg-social-share' );
	...
	*/
	...
	public function catch_first_image() {
		...
		if ( is_singular( ) ) {
			...
			if ( has_post_thumbnail( $post->ID ) ) {
				...
				$first_img = ( self::opt('us_pinterest_default', 'us_basic') ) ? self::opt('us_pinterest_default', 'us_basic'): plugins_url( 'hg-social-share/public/assets/img/ultimate_social.png' );
			}
		} else {
			$first_img = ( self::opt('us_pinterest_default', 'us_basic') ) ? self::opt('us_pinterest_default', 'us_basic'): plugins_url( 'hg-social-share/public/assets/img/ultimate_social.png' );
		}
	...
	public function mail_button($url) {
		...
		$body = ( self::opt('us_mail_body', 'us_mail') ) ? self::mail_replace_vars( self::opt('us_mail_body', 'us_mail'), $url ) : self::mail_replace_vars( __('I read this article and found it very interesting, thought it might be something for you. The article is called {post_title} and is located at {post_url}.','hg-social-share'), $url );

		$captcha_enable	= ( self::opt('us_mail_captcha_enable', 'us_mail') ) ? self::opt('us_mail_captcha_enable', 'us_mail') : 'yes';
		$captcha = ( self::opt('us_mail_captcha_question', 'us_mail') ) ? stripslashes( self::opt('us_mail_captcha_question', 'us_mail') ) : __('What is the sum of 7 and 2?','hg-social-share');

		$random_string = self::random_string(5);

		$us_share = __('Share with your friends','hg-social-share');
		$your_name = __('Your Name','hg-social-share');
		$your_email = __('Your Email','hg-social-share');
		$recipient_email = __('Recipient Email','hg-social-share');
		$your_message = __('Enter a Message','hg-social-share');
		$captcha_label = __('Captcha','hg-social-share');
	...
	public function us_send_mail(){
		...
		$subject		= ( self::opt('us_mail_subject', 'us_mail') ) ? self::mail_replace_vars( self::opt('us_mail_subject', 'us_mail'), $url ) : self::mail_replace_vars( __('A visitor of {site_title} shared {post_title} with you.','hg-social-share'), $url );
		...
		if ( $captcha_enable == 'yes' ){
			if( '' == $captcha )
				die( __( 'Captcha cannot be empty!', 'hg-social-share' ) );
			if( $captcha !== $captcha_answer )
				die( __( 'Captcha does not match.', 'hg-social-share' ) );
		}

		if ( ! filter_var( $recipient_email, FILTER_VALIDATE_EMAIL ) ) {
			die( __( 'Recipient email address is not valid.', 'hg-social-share' ) );
		} elseif ( ! filter_var( $your_email, FILTER_VALIDATE_EMAIL ) ) {
			die( __( 'Your email address is not valid.', 'hg-social-share' ) );
		} elseif( strlen( $your_name ) == 0 ) {
			die( __( 'Your name cannot be empty.', 'hg-social-share' ) );
		} elseif( strlen( $message ) == 0 ) {
			die( __( 'Message cannot be empty.', 'hg-social-share' ) );
		}
		$headers	= array();
		...
		if ( ! $result ) {
			...
			if( isset( $phpmailer->ErrorInfo ) ) {
				...
				die( __( 'Unknown wp_mail() error.', 'hg-social-share' ) );
			}
		}
	}
